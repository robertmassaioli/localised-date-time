...
Replace with explicit `Number.isInteger` / `isNaN` validation:
...
The `<Label>` components use `labelFor` props that reference field names (e.g. `labelFor='time'`), but the corresponding `<Textfield>` and `<Select>` components only have a `name` prop, not an `id`. In standard HTML, `<label for="x">` requires the target element to have `id="x"`, not `name="x"`. Depending on how Forge UI Kit renders these, the label association may be broken for assistive technology users.

Additionally, there is no `isRequired` on the `time` and `date` fields even though they are mandatory for the macro to function.

### Proposed Fix

- Audit Forge UI Kit docs to confirm whether `labelFor` binds to `name` or `id`; if the latter, add matching `id` props to all fields.
- Add `isRequired` to the `time` `<Textfield>` and `date` `<DatePicker>` components in both macros to provide client-side validation hints and correct ARIA semantics.
...
| 6 | Fix `parseInt`/`isPresent` silent failure for repetition period | Security / Robustness | XS | ⬜ Pending |
